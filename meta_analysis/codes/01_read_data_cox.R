...
if (z_age_gp=="5-11" | z_age_gp=="5-17") z_age_gp <- paste0("0",z_age_gp)
z_age_gp <- gsub("-","_",z_age_gp)

#if (z_title == "Hospital Death Age 5-15") ni_ests <- read.csv(paste0("data/ni/m_hosp_death_05_17_coef_",z_model,".csv"))
#if (z_title == "Hospital Death Age 16-74") ni_ests <- read.csv(paste0("data/ni/m_hosp_death_18_74_coef_",z_model,".csv"))
#if (z_title == "Hospital Death Age 75+") ni_ests <- read.csv(paste0("data/ni/m_hosp_death_75plus_coef_",z_model,".csv"))
...
for (i in names(coefs)){
  ...
  if(i =="england") z_df <- z_df %>% filter(!is.na(coef)) %>% 
...
for (i in names(vcovs)){
  ...
  names(z_df) <- z_names}
  if(i == "wales") {z_names <- z_df$term
  z_df <- z_df %>% dplyr::select(-term)
  names(z_df) <- z_names}
...
if (z_title == "Hospital Death Age 5-15") {
  z <- z %>% 
    mutate(xvar = if_else(country=="ni" & xvar=="qc_count_cat", "qc_count_cat3", xvar)) %>% 
    mutate(xlbl = if_else(country=="ni" & xvar=="qc_count_cat3" & xlbl == "2", "2plus", xlbl))
}
  
df_coefs <- z %>% arrange(country, xvar, xlbl)

if (z_title == "Hospital Death Age 5-15") df_coefs <- df_coefs %>% filter(country %in% c("england","scotland"))
