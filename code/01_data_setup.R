...
eave_rg = readRDS(paste0(Location,"EAVE/GPanalysis/outputs/temp/CR_Cohort_RG_EAVE_BP_Smoke.rds")) %>%
  ...
  mutate(EAVE_Smoke = as.character(EAVE_Smoke),
         EAVE_BP = as.character(EAVE_BP)) %>%
  mutate(EAVE_Smoke = case_when(EAVE_Smoke == 'Unknown' ~ NA_character_,
                                TRUE ~ EAVE_Smoke),
         EAVE_BP = case_when(EAVE_BP == 'No Investigation' ~ NA_character_,
                             TRUE ~ EAVE_BP)) %>%
  mutate(EAVE_Smoke = as.factor(EAVE_Smoke),
...
Vaccinations = Vaccinations %>%
  ...
  mutate(vacc_product_name = gsub("Covid-19 Vaccine AstraZeneca", "AZ", vacc_product_name),
  ...
  pivot_wider(id_cols = c("EAVE_LINKNO", "first_dose", "second_dose", "third_dose", "fourth_dose", "fifth_dose"), 
              names_prefix = "vacc_type_", names_from = vacc_dose_number, 
              values_from = vacc_product_name, values_fill = NA, values_fn = Mode) %>%
  rename(date_vacc_1 = first_dose,
...
duplicate_vaccs = filter(Vaccinations, EAVE_LINKNO %in% duplicate_vacc_ids) %>%
  ...
  mutate_at( setdiff(names(Vaccinations), c('EAVE_LINKNO', 'shielding')), Mode) %>%
  distinct 
...
endpoints = covid_hospitalisations %>%
  ...
  mutate(covid_hosp_death = case_when(covid_hosp == 1 | covid_death == 1 ~ 1,
                                      TRUE ~ 0)) %>%
  mutate(covid_hosp_death_date = case_when(covid_hosp_death == 1 ~ pmin(covid_hosp_date, covid_death_date, na.rm = TRUE),
                                           TRUE ~ as.Date(NA))) %>%
  distinct()
...
df_cohort = EAVE_cohort %>%
  ...
  mutate(ageYear = ageYear + 2) %>%
  filter(ageYear >= 5) %>%
  mutate(age_gp = cut(ageYear, breaks = c(5, 16, 76, Inf), 
                      labels = c('5-15', '16-75', '75+'),
                      right = FALSE)) %>%
  full_join(EAVE_Weights, by="EAVE_LINKNO")
...
df_cohort = mutate(df_cohort, bmi_cat = cut(Q_BMI, breaks =  c(10, 18.5, 25, 30, 35, 40, Inf),
...
df_cohort = left_join(df_cohort, Vaccinations, by = 'EAVE_LINKNO') %>%
  mutate(
    num_doses = case_when( 
      ...
      !is.na(date_vacc_1) ~ 1,
      TRUE ~ 0),
    vacc_seq = paste0( ifelse(is.na(vacc_type_1), '', paste0(vacc_type_1, '_')),
    ...
    mixed_vaccines = case_when(
      ...
      TRUE ~ 0)) %>%
  mutate(vacc_seq = trimws(vacc_seq, whitespace = '_')) %>%
  mutate(vacc_seq = case_when(vacc_seq == '' ~ 'uv',
                              TRUE ~ vacc_seq))

vacc_seq_count = count(df_cohort, vacc_seq)
vacc_seq_count_pruned = filter(vacc_seq_count, n>=1000)

# Add household characteristics
df_cohort = left_join(df_cohort, Cohort_Household, by = 'EAVE_LINKNO')

# Add deaths from any cause
df_cohort = left_join(df_cohort, all_deaths, by = 'EAVE_LINKNO')

# Add in covid hospitalisations and deaths
df_cohort = endpoints %>% 
  group_by(EAVE_LINKNO) %>%
  ...
  mutate(covid_hosp_ever = case_when(covid_hosps >= 1 ~ 1,
                                     TRUE ~ 0)) %>%
  left_join(all_deaths %>% 
              select(EAVE_LINKNO, covid_death)) %>%
...
df_cohort = filter(cdw, date_ecoss_specimen >= study_start %m-% months(6)) %>%
  ...
  mutate(num_tests_6m_group = cut(num_tests, breaks = c(0, 1, 2, 3, 4, 10, Inf),
...
df_cohort = mutate(df_cohort, vs = case_when( date_vacc_4 <= study_start ~ paste0('v4_', vacc_type_4),
                                              ...
                                              TRUE ~ 'uv'  )) %>%
  mutate(fully_vaccinated = case_when(age_gp == '5-15' & date_vacc_2 <= study_start ~ 1,
                                      age_gp == '16-75' & date_vacc_3 <= study_start ~ 1,
                                      age_gp == '75+' & date_vacc_4 <= study_start ~ 1,
                                      TRUE ~ 0),
         vs_mixed = case_when(mixed_vaccines == 1 & !(vacc_seq %in% vacc_seq_count_pruned$vacc_seq) ~ paste0('v', num_doses, '_mixed'),
                                                           TRUE ~ vacc_seq)) %>%
...
df_cohort = mutate_at(df_cohort, cols, ~case_when( is.na(.) ~ 0,
                                                   TRUE ~ .)) %>%
  mutate(Q_HOME_CAT = case_when(Q_HOME_CAT == 0 ~ 'Neither',
                                Q_HOME_CAT == 1 ~ 'Care home',
                                Q_HOME_CAT == 2 ~ 'Homeless'),
         Q_LEARN_CAT = case_when(Q_LEARN_CAT == 0 ~ 'Neither',
                                 Q_LEARN_CAT == 1 ~ 'Learning disability',
                                 Q_LEARN_CAT == 2 ~ 'Down\'s syndrome'),
         Q_DIAG_CKD_LEVEL = case_when(Q_DIAG_CKD_LEVEL == 0 ~ 'No CKD',
...
df_cohort = mutate(df_cohort, n_risk_gps = as.character(n_risk_gps)) %>%
  mutate(n_risk_gps = case_when( is.na(n_risk_gps) ~ '0',
                                 TRUE ~ n_risk_gps) ) %>%
  mutate(n_risk_gps = as.factor(n_risk_gps))


df_cohort = mutate_at(df_cohort, c('EAVE_Smoke', 'EAVE_BP'), ~as.character(.)) %>%
  mutate(EAVE_Smoke = case_when(EAVE_Smoke == 'Ex Smoker' ~ 'Ex-smoker',
                                EAVE_Smoke == 'Non Smoker' ~ 'Non-smoker',
                                TRUE ~ EAVE_Smoke),
         EAVE_BP = case_when(EAVE_BP == 'Very High' ~ 'Very high',
                             TRUE ~ EAVE_BP),
         ur6_2016_name = case_when(ur6_2016_name == '1 Large Urban Areas' ~ 'Large Urban Areas',
  ...
  mutate(EAVE_Smoke = fct_relevel(EAVE_Smoke, 'Non-smoker'),
         ...
         ur6_2016_name = fct_relevel(ur6_2016_name, 'Large Urban Areas',
                                     'Other Urban Areas', 
                                     'Accessible Small Towns',
                                     'Remote Small Towns',
                                     'Accessible Rural',
                                     'Remote Rural'),
         simd2020_sc_quintile = as.factor(simd2020_sc_quintile)
  )
